    ...
    for i in range(heroes_per_player):
        ...
        for j in range(len(spiders)):
            if spiders[j]["threat"] == 1 and spiders[j]["targeted"] == 0:
                s_loc = [spiders[j]["x"], spiders[j]["y"]]
                h_loc = [heroes[i]["x"], heroes[i]["y"]]
                if math.dist(s_loc, h_loc) < min_distance:
                    s = spiders[j]
                    min_distance = math.dist(s_loc, h_loc)
        ...
        if min_distance < 1280 and mymana > 10:
            action = f"SPELL WIND 8800 4400 wooosh"

        # Modified changes to ensure only the first two heroes will attack
        if i == 0 or i == 1:
            for j in range(len(spiders)):
                if spiders[j]["threat"] == 1 and spiders[j]["targeted"] == 0:
                    action += f" ATTACK {spiders[j]['id']};"
                    spiders[j]["targeted"] = 1

        if action == "":
            action = "WAIT i'm waiting"
        print(action)
